    ...
    function load() {
       ...
       $unidad=pg_query($con,"SELECT  unidad.uniarea, unidad.uninombre, unidad.uniid, unidad.kml FROM unidad 
        ...
        while ($reg = pg_fetch_array($unidad)) {
            ...
            'BATERIAS SANITARIAS CASINO', 'BATERIAS SANITARIA DISCAPACITADOS CASINO', 'BAÑOS MUJERES CASINO', 'BAÑOS HOMBRES CASINO', 'BATERIA SANITARIAS BIOTECNOLOGIA UNO',
            'BATERIAS SANITARIAS BIOTECNOLOGIA DOS', 'BATERIAS SANITARIAS DISCAPACITADOS TECNOPARQUE', 'BATERIAS SANITARIAS TECNOPARQUE', 'BATERIAS SANITARIAS AGRICULTURA DE PRECISION',
           ...
           inner join unidad on unidad.uniid = construccion.conunidad
           inner join area on area.areid = unidad.uniarea
           WHERE arenombre IN ( 'GESTION', 'GESTION DE CENTRO') AND connombre  IN ('AMBIENTE A1', 'AMBIENTE A2', 'AMBIENTE A3', 'AMBIENTE A4', 'AMBIENTE AGRONICA Y MECATRONICA', 'AMBIENTE AUTOAGRONOMO',
           'AMBIENTE B1', 'AMBIENTE B2', 'AMBIENTE C1', 'AMBIENTE D1', 'AMBIENTE D2', 'AMBIENTE E1', 'AMBIENTE E2', 'AMBIENTE E3', 'AMBIENTE E4', 'AMBIENTE GUADUA-GM', 'AMBIENTE H1', 'AMBIENTE H2',
           'AMBIENTE H3', 'AMBIENTE H4', 'AMBIENTE J1', 'AMBIENTE J2', 'AMBIENTE L1', 'AMBIENTE L2', 'AMBIENTE L3', 'AMBIENTE N1', 'AMBIENTE N2', 'AMBIENTE N3', 'AMBIENTE N4', 'AMBIENTE N5', 'MERCA SENA',
           'OFICINA SENA EMPRESA', 'BIBLIOTECA', 'SALON ESMERALDA', 'LABORATORIO DE SUELOS', 'BIENESTAR APRENDIZ', 'BLOQUE ADMINISTRATIVO', 'OFICINA ENLACE RACK', 'SALA DE INSTRUCTORES')");

            $cabañas = pg_query($con, "SELECT  construccion.conidcodigo, construccion.connombre,  construccion.conunidad , construccion.kml FROM construccion
            inner join unidad on unidad.uniid = construccion.conunidad
            inner join area on area.areid = unidad.uniarea
            WHERE arenombre IN ( 'GESTION', 'GESTION DE CENTRO') AND connombre  IN ('CABAÑA 1', 'CABAÑA 2', 'CABAÑA 3', 'CABAÑA 4')");
            ...
            inner join unidad on unidad.uniid = construccion.conunidad
            inner join area on area.areid = unidad.uniarea
            WHERE arenombre IN ( 'GESTION', 'GESTION DE CENTRO') AND connombre  IN ('KIOSKO 1', 'KIOSKO 2', 'KIOSKO INSTRUCTORES')");
            ...
            while ($reg1 = pg_fetch_array($construcion)) {
                echo "
                            ...
                            strategies: [new OpenLayers.Strategy.Fixed()],
                            protocol: new OpenLayers.Protocol.HTTP({
                                url: 'kml/KmlZonasComunes/$reg1[3]',
                                format: new OpenLayers.Format.KML({
                                    extractStyles: true,
                                    extractAttributes: true,
                                    maxDepth: 2
            ...
            while ($reg1 = pg_fetch_array($edificios)) {
              echo "
                          var ".str_replace(array('-' , ' ' ), "", $reg1[1])." = new OpenLayers.Layer.Vector('$reg1[1]', {
                          strategies: [new OpenLayers.Strategy.Fixed()],
                          protocol: new OpenLayers.Protocol.HTTP({
          ...
          while ($reg1 = pg_fetch_array($cabañas)) {
            echo "
                        var ".str_replace(array('-' , ' ' ), "", $reg1[1])." = new OpenLayers.Layer.Vector('$reg1[1]', {
                        strategies: [new OpenLayers.Strategy.Fixed()],
                        protocol: new OpenLayers.Protocol.HTTP({
                      ...
                      "var group = [".str_replace(array('-' , ' ' ), "", $reg1[1])."];
                      for (var i=0; i < group.length; i++) { group[i].group = 'CABAÑAS'; }
                      map.addLayers([".str_replace(array('-' , ' ' ), "", $reg1[1])."]);
                      ".str_replace(array('-' , ' ' ), "", $reg1[1]).".setVisibility(false);".
                      "";
        }
      
        while ($reg1 = pg_fetch_array($zonascomunes)) {
          echo "
                      var ".str_replace(array('-' , ' ' ), "", $reg1[1])." = new OpenLayers.Layer.Vector('$reg1[1]', {
                      strategies: [new OpenLayers.Strategy.Fixed()],
                      protocol: new OpenLayers.Protocol.HTTP({
        ...
        var fpControl = new OpenLayers.Control.FeaturePopups({
            ...
            <?php
                ...
                while ($reg = pg_fetch_array($unidad)) { 
                    ...
                    inner join unidad on unidad.uniid = construccion.conunidad
                    inner join area on area.areid = unidad.uniarea
                    WHERE arenombre IN ( 'GESTION', 'GESTION DE CENTRO') AND connombre  IN ('BATERIAS SANITARIAS INSTRUCTORES')");
            ...
            single: "<table border='0'><tr><td width='200px'><center><h5><?php echo $reg1[1] ;?></h5></td></tr><tr><td width='200px'><img src='img/lot_8.png' class='LotesCultivos'></td></tr><tr><td><center><input type='submit' value='Ver m&aacute;s informaci&oacute;n' onclick='Construccion(`<?php echo $reg1[1] ;?>`)'></center></td></tr></table>",
        ...
        <?php
            ...
            [   
                ...
                <?php echo str_replace(array('-' , ' ' ), '', $reg[1]);?> , {templates: {
                    single: "<table border='0'><tr><td width='200px'><center><h5><?php echo $reg[1] ;?></h5></td></tr><tr><td width='200px'><img src='img/lot_8.png' class='LotesCultivos'></td></tr><tr><td><center><input type='submit' value='Ver m&aacute;s informaci&oacute;n' onclick='UnidadP(`<?php echo $reg[1] ;?>`)'></center></td></tr></table>",
...
<ul>
    <button class="ZONASCOMUNES">
      <li> <a href="Zonas_Comunes3.php" class="menu" ><font color="black">Gestion de Centro</a></li> 
  </button>
</ul>
</UL>
</div>
