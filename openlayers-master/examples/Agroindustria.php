    ...
    function load()
     {
        ...
        var PolAdmin = new OpenLayers.Layer.Vector("Construcciones", {
            strategies: [new OpenLayers.Strategy.Fixed()],
            protocol: new OpenLayers.Protocol.HTTP({
                url: "kml/KmlAgroindustria/Agroindustria.kml",
                format: new OpenLayers.Format.KML({
                    extractStyles: true, 
                    extractAttributes: true,
                    maxDepth: 2
        ...
        // });
         ...
         var PolAdminPo = new OpenLayers.Layer.Vector("Postes", {
            strategies: [new OpenLayers.Strategy.Fixed()],
            protocol: new OpenLayers.Protocol.HTTP({
                url: "kml/kmlPostes/PostesAgroindustria.kml",
                format: new OpenLayers.Format.KML({
                    extractStyles: true,
                    extractAttributes: true,
                    maxDepth: 2
        ...
        map.addLayers([PolAdminPo]);
          ...
          var PoligonoCa= new OpenLayers.Layer.Vector("Canales", {
            strategies: [new OpenLayers.Strategy.Fixed()],
            protocol: new OpenLayers.Protocol.HTTP({
                url: "kml/kmlCanales/CanalAgroindustria.kml",
                format: new OpenLayers.Format.KML({
                    extractStyles: true, 
                    extractAttributes: true,
                    maxDepth: 2
           ...
           var PoligonoCr= new OpenLayers.Layer.Vector("Carreteables", {
            strategies: [new OpenLayers.Strategy.Fixed()],
            protocol: new OpenLayers.Protocol.HTTP({
                url: "kml/kmlCarreteables/CarreteableAgroindustria.kml",
                format: new OpenLayers.Format.KML({
                    extractStyles: true, 
                    extractAttributes: true,
                    maxDepth: 2
           ...
           map.addLayers([PoligonoCa, PoligonoCr]);
        <?php
...
while ($reg = pg_fetch_array($unidad)) {
    ...
    echo "
       ...
       var " .str_replace(array('-' , ' ' ), "", $reg[1]). " = new OpenLayers.Layer.Vector('Area Total', {
        strategies: [new OpenLayers.Strategy.Fixed()],
        protocol: new OpenLayers.Protocol.HTTP({
            url: 'kml/KmlAgroindustria/$reg[3]',
            format: new OpenLayers.Format.KML({
                extractStyles: true,
                extractAttributes: true,
                maxDepth: 2
    ...
    //                 strategies: [new OpenLayers.Strategy.Fixed()],
    //                 protocol: new OpenLayers.Protocol.HTTP({
    //                     url: 'kml/KmlAgroindustria/$reg1[3]',
    //                     format: new OpenLayers.Format.KML({
    //                         extractStyles: true,
    //                         extractAttributes: true,
    //                         maxDepth: 2
...
<?php
    ...
    [   
        ...
        <?php echo str_replace(array('-' , ' ' ), '', $reg[1]);?> , {templates: {
            single: "<table border='0'><tr><td width='200px'><center><h5><?php echo $reg[1] ;?></h5></td></tr><tr><td width='200px'><img src='img/lot_8.png' class='LotesCultivos'></td></tr><tr><td><center><input type='submit' value='Ver m&aacute;s informaci&oacute;n' onclick='Unidad(`<?php echo $reg[1] ;?>`)'></center></td></tr></table>",
 ...
 <div id="mapdiv" class="smallmap" style="position: relative;" onload="load()"></div>
   ...
   <!--  <script type="text/javascript">
      ...
      jQuery(document).ready(function(){
        ...
        $("#OpenLayers_Control_LayerSwitcher_274_layersDiv").scroll():
       // document.getElementById("OpenLayers_Control_LayerSwitcher_274_layersDiv").style.overflow="auto";
      }) 
    </script> -->