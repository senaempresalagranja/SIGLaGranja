...
class  MiPDF extends FPDF 
{
	...
	public function Header()
	{
		...
		$this -> Cell( 180, 20 , "BD: siglagranja / Plaga-Especie   ", 0 , 0 , 'C' );						
...
$peskidcodigo = array("CODIGO");
$pesespecie = array("ESPECIE");
$pesplaga = array("PLAGA");
$mipdf -> addPage('A5' , 'letter',20,39);////Orientación de la pagina (Horizontalmente)

for ($i = 0; $i < count( $peskidcodigo ) ; $i++)
{
	$mipdf -> SetFont( 'Arial' , 'B' , 10);
	$mipdf -> SetTextColor( 255, 255 ,255);
	$mipdf -> SetFillColor(  0 , 0 ,255);
	$mipdf -> Cell ( 30, 10 , $peskidcodigo[ $i ] , 1 , 0 , 'C' , true );
}
for ($i = 0; $i < count( $pesespecie ) ; $i++)
{
	$mipdf -> SetFont( 'Arial' , 'B' , 10);
	$mipdf -> SetTextColor( 255, 255 ,255);
	$mipdf -> SetFillColor(  0 , 0 ,255);
	$mipdf -> Cell ( 90, 10 , $pesespecie[ $i ] , 1 , 0 , 'C' , true );	
}
for ($i = 0; $i < count( $pesplaga ) ; $i++)
{
	$mipdf -> SetFont( 'Arial' , 'B' , 10);
	$mipdf -> SetTextColor( 255, 255 ,255);
	$mipdf -> SetFillColor(  0 , 0 ,255);
	$mipdf -> Cell ( 90, 10 , $pesplaga[ $i ] , 1 , 0 , 'C' , true );	
}
$mipdf -> Ln( 10);
...
$res=pg_query($con, "SELECT * FROM plaga_especie ORDER BY peskidcodigo ASC");
			while($reg=pg_fetch_array($res))
			{
				...
				$pesdescripci=utf8_decode($reg[4]);
	
	$mipdf -> SetFont( 'Arial' , 'B' , 10);
	$mipdf -> SetTextColor(0,0,0);
	$mipdf -> SetFillColor( 255,255,255);		
	$mipdf -> Cell( 30, 10 , $codigo, 1, 0, 'R' , true );
	$mipdf -> Cell( 90, 10 , $nomcomun, 1, 0, 'L' , true );	
	$mipdf -> Cell( 90, 10 , $nomcomun1, 1, 0, 'L' , true );
	$mipdf -> Ln( 10);	
}   
$mipdf -> Output('Pdf_PlagaEspecie.pdf','I');//Nombre de descarga y Tipo de Visualización
?>
